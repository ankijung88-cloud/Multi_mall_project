...
export default function KAuditionSection() {
    ...
    const sectionPartners = partners.filter(p => p.category === '오디션').slice(0, 3);

    return (
        <section className="py-20 border-b" style={{
            background: 'linear-gradient(135deg, rgba(255, 254, 249, 0.5) 0%, rgba(250, 243, 224, 0.5) 100%)',
            borderColor: '#CBD5E0'
        }}>
            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                <div className="flex items-center justify-between mb-12">
                    <h2 className="text-3xl font-hanbok pl-4" style={{
                        color: '#4A5568',
                        borderLeft: '4px solid #8E9AAF'
                    }}>
                        K-AUDITION
                    </h2>
                    <button
                        onClick={() => navigate('/partners/audition' + linkParams)}
                    ...
                    {sectionPartners.map((partner) => (
                        ...
                        >
                            <motion.div
                                whileHover={{ y: -5 }}
                            >
                                <div className="w-full aspect-[4/3] rounded-2xl overflow-hidden border border-gray-100 group-hover:border-yellow-500 transition-all mb-3 shadow-sm">
                                    <img
                                        src={partner.image}
                                        alt={partner.name}
                                ...
                                <h3 className="font-bold text-lg text-gray-800 group-hover:text-yellow-600 truncate px-1">
                                    {partner.name}
                                </h3>
                            </motion.div>
                        </Link>
